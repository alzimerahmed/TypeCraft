# TypeCraft Features & Setup Guide

TypeCraft combines a lightweight, privacy-focused keyboard foundation with cutting-edge productivity tools: **Multi-Provider Cloud & Offline AI**, **On-Device Whisper Voice Typing**, **Handwriting Recognition**, **Dual-Engine In-Keyboard Translation**, **Rich Text Utilities**, and **Deep UI Customization**.
...
TypeCraft connects directly with top AI providers for ultra-fast proofreading, grammar corrections, tone adjustments, and rewrites.
...
2. In TypeCraft, open **Settings → AI Integration → Set AI Provider**.
...
> [!IMPORTANT]
> **Zero-Network Guarantee**: This feature runs 100% locally via the companion [**TypeCraft Offline AI Plugin**](https://github.com/alzimerahmed84/TypeCraft-Offline-AI-Plugin) powered by `llama.cpp` and is available in the **Offline** build flavor (`-offline-release.apk`). No internet permission exists in the manifest.
...
2. In TypeCraft, open **Settings → Plugins → Offline AI** and tap **Load Offline AI plugin** to load the `.apk`.
...
TypeCraft offers a flexible translation architecture supporting all app flavors:

1. **Translation Plugin** (Supported across all flavors):
   - High-speed, private translation powered by the companion [TypeCraft Translation Plugin](https://github.com/alzimerahmed84/TypeCraft-Translation-Plugin/releases/latest).
...
TypeCraft integrates high-accuracy, private speech-to-text powered by OpenAI's Whisper architecture via `whisper.cpp` and the [TypeCraft Voice Plugin](https://github.com/alzimerahmed84/TypeCraft-Voice-Plugin).
...
Draw letters, words, or symbols directly on a handwriting recognition canvas using your finger or stylus via the companion [TypeCraft Handwriting Plugin](https://github.com/alzimerahmed84/TypeCraft-Handwriting-Plugin) (supported across all flavors).

### Setup Instructions
1. **Online Flavors**: Open **Settings → Handwriting** and tap **Download Plugin** to install the [TypeCraft Handwriting Plugin](https://github.com/alzimerahmed84/TypeCraft-Handwriting-Plugin/releases/latest).
...
Detach TypeCraft into a moveable, resizable floating window:
...
TypeCraft features a comprehensive, privacy-first clipboard manager with rich gestural editing:
...
- **Dynamic Messaging App Selector**: Choose which specific messaging apps (Google Messages, Signal, WhatsApp, Telegram, etc.) TypeCraft should monitor for OTP codes.
...
TypeCraft learns your vocabulary organically as you type:
...
TypeCraft is published in **3 purpose-built flavors**:
...
> [!TIP]
> **Concurrent Installation**: The `offline` (`com.Alzimer Ahmed.TypeCraft.offline`) build uses a unique package ID, allowing you to install it alongside `standardfull` on the same device!

---

## 24. Offline Camera OCR & Screenshot Extraction

TypeCraft features an on-device OCR engine powered by ML Kit via the [TypeCraft OCR Plugin](https://github.com/alzimerahmed84/TypeCraft-Ocr-Plugin) (supported across all flavors), enabling instant text extraction from live camera feeds or captured screenshots with zero internet connectivity.
...
TypeCraft includes a zero-latency native keypress audio feedback engine that delivers rich auditory tactile response:
...
- **Remote Catalog**: Download additional physical modeling and synthesized instrument packs on demand from the official GitHub sound pack repository (`alzimerahmed84/TypeCraft-Sound-Packs`).
