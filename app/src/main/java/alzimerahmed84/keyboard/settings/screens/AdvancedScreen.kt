...
import alzimerahmed84.keyboard.keyboard.KeyboardActionListener
...
import alzimerahmed84.keyboard.keyboard.internal.keyboard_parser.POPUP_KEYS_ALL
import alzimerahmed84.keyboard.keyboard.internal.keyboard_parser.POPUP_KEYS_MAIN
import alzimerahmed84.keyboard.keyboard.internal.keyboard_parser.POPUP_KEYS_MORE
import alzimerahmed84.keyboard.keyboard.internal.keyboard_parser.POPUP_KEYS_NORMAL
import alzimerahmed84.keyboard.keyboard.internal.keyboard_parser.morePopupKeysResId
import alzimerahmed84.keyboard.latin.BuildConfig
import alzimerahmed84.keyboard.latin.R
import alzimerahmed84.keyboard.latin.SystemBroadcastReceiver
import alzimerahmed84.keyboard.latin.common.splitOnWhitespace
import alzimerahmed84.keyboard.latin.settings.DebugSettings
import alzimerahmed84.keyboard.latin.settings.Defaults
import alzimerahmed84.keyboard.latin.settings.Settings
import alzimerahmed84.keyboard.latin.utils.checkTimestampFormat
...
import alzimerahmed84.keyboard.settings.SettingsContainer
...
import alzimerahmed84.keyboard.settings.dialogs.TextInputDialog
import alzimerahmed84.keyboard.settings.dialogs.ListPickerDialog
...
import alzimerahmed84.keyboard.settings.preferences.BackupRestorePreference
...
import alzimerahmed84.keyboard.settings.FeedbackManager
...
fun createAdvancedSettings(context: Context) = listOfNotNull(
    ...
    Setting(context, SettingsWithoutKey.GEMINI_MODEL, R.string.gemini_model_title, R.string.gemini_model_summary) { setting ->
        val ctx = LocalContext.current
        val service = remember { alzimerahmed84.keyboard.latin.utils.ProofreadService(ctx) }
        var items by remember { mutableStateOf(alzimerahmed84.keyboard.latin.utils.ProofreadService.AVAILABLE_MODELS.map { it to it }) }
        var selectedModel by remember { mutableStateOf(service.getModelName()) }

        LaunchedEffect(Unit) {
            val models = service.fetchAvailableModels(alzimerahmed84.keyboard.latin.utils.ProofreadService.AIProvider.GEMINI)
    ...
    Setting(context, SettingsWithoutKey.AI_PROVIDER, R.string.ai_provider_title, R.string.ai_provider_summary) { setting ->
        ...
        ListPreference(
            ...
            onChanged = { newProvider ->
                service.setProvider(alzimerahmed84.keyboard.latin.utils.ProofreadService.AIProvider.valueOf(newProvider))
    ...
    Setting(context, SettingsWithoutKey.GROQ_MODEL, R.string.huggingface_model_title, R.string.huggingface_model_summary) { setting ->
        val ctx = LocalContext.current
        val service = remember { alzimerahmed84.keyboard.latin.utils.ProofreadService(ctx) }
        var items by remember { mutableStateOf(alzimerahmed84.keyboard.latin.utils.GroqModels.AVAILABLE_MODELS.map { it to it }) }
        var selectedModel by remember { mutableStateOf(service.getGroqModel()) }

        LaunchedEffect(Unit) {
            val models = service.fetchAvailableModels(alzimerahmed84.keyboard.latin.utils.ProofreadService.AIProvider.GROQ)
            items = models.map { it to it }
        }
    ...
    Setting(context, SettingsWithoutKey.GEMINI_TARGET_LANGUAGE, R.string.translate_target_language_title, R.string.translate_target_language_summary) { setting ->
        val ctx = LocalContext.current
        val service = remember { alzimerahmed84.keyboard.latin.utils.ProofreadService(ctx) }
        val languageNames = ctx.resources.getStringArray(alzimerahmed84.keyboard.latin.R.array.translate_language_names)
        val languageCodes = ctx.resources.getStringArray(alzimerahmed84.keyboard.latin.R.array.translate_language_codes)
        ...
        val items = remember(selectedLanguage, listVersion) {
            ...
            val history = alzimerahmed84.keyboard.latin.utils.TranslationUtils.getLanguageHistory(ctx.prefs())
            val removed = alzimerahmed84.keyboard.latin.utils.TranslationUtils.getRemovedLanguages(ctx.prefs())
            ...
            for (h in history.reversed()) {
                if (h.first.lowercase() !in removed && h.second.lowercase() !in removed && filteredZipped.none { alzimerahmed84.keyboard.latin.utils.TranslationUtils.isSameLanguage(it, h) }) {
        ...
        alzimerahmed84.keyboard.settings.preferences.Preference(
                            ...
                            androidx.compose.foundation.layout.Row(
                                modifier = androidx.compose.ui.Modifier
                                    ...
                                    .clickable {
                                        ...
                                        alzimerahmed84.keyboard.latin.utils.TranslationUtils.saveLanguageHistory(ctx.prefs(), name, code)
                                ...
                                androidx.compose.material3.IconButton(
                                    onClick = {
                                        alzimerahmed84.keyboard.latin.utils.TranslationUtils.removeLanguageHistory(ctx.prefs(), code)
                                        if (isSelected) {
                                            val fallback = "en"
            ...
            TextInputDialog(
                ...
                onConfirmed = { customLang ->
                    ...
                    if (trimmed.isNotEmpty()) {
                        ...
                        alzimerahmed84.keyboard.latin.utils.TranslationUtils.saveLanguageHistory(ctx.prefs(), trimmed, trimmed)
    ...
    Setting(context, SettingsWithoutKey.TRANSLATE_GEMINI_MODEL, R.string.translate_model_title, R.string.translate_model_summary) { setting ->
        val ctx = LocalContext.current
        val service = remember { alzimerahmed84.keyboard.latin.utils.ProofreadService(ctx) }
        var items by remember { mutableStateOf(listOf("Default (Proofreading)" to "") + alzimerahmed84.keyboard.latin.utils.ProofreadService.AVAILABLE_MODELS.map { it to it }) }
    ...
    Setting(context, SettingsWithoutKey.TRANSLATE_GROQ_MODEL, R.string.translate_model_title, R.string.translate_model_summary) { setting ->
        val ctx = LocalContext.current
        val service = remember { alzimerahmed84.keyboard.latin.utils.ProofreadService(ctx) }
        var items by remember { mutableStateOf(listOf("Default (Proofreading)" to "") + alzimerahmed84.keyboard.latin.utils.GroqModels.AVAILABLE_MODELS.map { it to it }) }
    ...
    if (BuildConfig.FLAVOR == "offline") Setting(context, SettingsWithoutKey.LOAD_OFFLINE_AI_PLUGIN, R.string.load_offline_ai_plugin, R.string.load_offline_ai_plugin_summary) {
        alzimerahmed84.keyboard.settings.preferences.LoadOfflineAiPluginPreference(
            ...
            summary = if (alzimerahmed84.keyboard.latin.ai.OfflineAiLoader.hasPlugin(LocalContext.current)) "Plugin active (version ${alzimerahmed84.keyboard.latin.ai.OfflineAiLoader.getPluginVersion(LocalContext.current) ?: "1.0"})" else stringResource(R.string.load_offline_ai_plugin_summary),
    ...
    if (BuildConfig.FLAVOR == "offline") Setting(context, SettingsWithoutKey.OFFLINE_MODEL_PATH, R.string.offline_model_title, R.string.offline_model_summary) { setting ->
        ...
        val service = remember { alzimerahmed84.keyboard.latin.utils.ProofreadService(context) }
