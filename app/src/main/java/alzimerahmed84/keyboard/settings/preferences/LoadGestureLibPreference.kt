...
import alzimerahmed84.keyboard.latin.utils.ChecksumCalculator
import alzimerahmed84.keyboard.latin.utils.GestureLibraryDownloader
...
fun LoadGestureLibPreference(
    ...
    fun renameToLibFileAndRestart(file: File, checksum: String) {
        ...
        file.delete()
        try {
            ctx.sendBroadcast(Intent(alzimerahmed84.keyboard.dictionarypack.DictionaryPackConstants.NEW_DICTIONARY_INTENT_ACTION))
        } catch (e: Exception) { }
        onSuccess?.invoke()
        isDownloading = false
        showDialog = false
    ...
    if (showDialog) {
        ...
        alzimerahmed84.keyboard.settings.dialogs.PreferenceDialog(
                        ...
                        if (!isInstalled) {
                            if (hasInternet) {
                                ...
                                Button(
                                    onClick = {
                                        showDialog = false
                                        val url = GestureLibraryDownloader.getDownloadUrl() ?: "https://github.com/alzimerahmed84/TypeCraft/releases"
