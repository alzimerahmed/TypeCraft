...
class AppsManager(val context: Context) : BroadcastReceiver() {
    ...
    fun registerForUpdates(listener: AppsChangedListener) {
        ...
        val useApps = context.prefs().getBoolean(alzimerahmed84.keyboard.latin.settings.Settings.PREF_USE_APPS, alzimerahmed84.keyboard.latin.settings.Defaults.PREF_USE_APPS)
        if (!useApps) return
