...
class ProofreadService(private val context: Context) {
    ...
    private fun fetchGeminiModels(): List<String> {
        ...
        return try {
            ...
            connection.setRequestProperty("User-Agent", "TypeCraft/1.0")
            connection.connect()
    ...
    fun isAllowInsecureConnections(): Boolean =
        context.prefs().getBoolean(
            alzimerahmed84.keyboard.latin.settings.Settings.PREF_AI_ALLOW_INSECURE_CONNECTIONS,
            alzimerahmed84.keyboard.latin.settings.Defaults.PREF_AI_ALLOW_INSECURE_CONNECTIONS
        )

    fun getCloudMaxTokens(): Int =
        context.prefs().getInt(
            alzimerahmed84.keyboard.latin.settings.Settings.PREF_CLOUD_AI_MAX_TOKENS,
            alzimerahmed84.keyboard.latin.settings.Defaults.PREF_CLOUD_AI_MAX_TOKENS
        )

    fun setCloudMaxTokens(tokens: Int) {
        context.prefs().edit().putInt(alzimerahmed84.keyboard.latin.settings.Settings.PREF_CLOUD_AI_MAX_TOKENS, tokens).apply()
    }
