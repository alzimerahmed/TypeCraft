...
class ShadowLocaleManagerCompat {
    companion object {
        @Implementation
        @JvmStatic
        fun getSystemLocales(context: Context): LocaleListCompat = LocaleListCompat.create(Locale.ENGLISH)
    }
}
...
class ShadowInputMethodManager2 : ShadowInputMethodManager() {
    ...
    override fun getInputMethodList() = listOf(
        if (BuildConfig.BUILD_TYPE == "debug" || BuildConfig.BUILD_TYPE == "debugNoMinify")
            InputMethodInfo("alzimerahmed84.keyboard.debug", "LatinIME", "TypeCraft debug", null)
        else InputMethodInfo("alzimerahmed84.keyboard", "LatinIME", "TypeCraft", null),
    )
    @Implementation
...
class ShadowBinaryDictionaryUtils {
    companion object {
        @Implementation
        @JvmStatic
