...
import alzimerahmed84.keyboard.ShadowInputMethodManager2
import alzimerahmed84.keyboard.ShadowLocaleManagerCompat
import alzimerahmed84.keyboard.event.Event
import alzimerahmed84.keyboard.keyboard.KeyboardSwitcher
import alzimerahmed84.keyboard.keyboard.MainKeyboardView
import alzimerahmed84.keyboard.keyboard.internal.keyboard_parser.floris.KeyCode
import alzimerahmed84.keyboard.latin.ShadowFacilitator2.Companion.addedWords
import alzimerahmed84.keyboard.latin.ShadowFacilitator2.Companion.lastAddedWord
import alzimerahmed84.keyboard.latin.ShadowFacilitator2.Companion.lastNgramContext
import alzimerahmed84.keyboard.latin.ShadowFacilitator2.Companion.ngramContexts
import alzimerahmed84.keyboard.latin.SuggestedWords.SuggestedWordInfo
...
import alzimerahmed84.keyboard.latin.dictionary.Dictionary
import alzimerahmed84.keyboard.latin.inputlogic.InputLogic
import alzimerahmed84.keyboard.latin.inputlogic.SpaceState
import alzimerahmed84.keyboard.latin.settings.Settings
import alzimerahmed84.keyboard.latin.utils.ScriptUtils
import alzimerahmed84.keyboard.latin.utils.SubtypeSettings
import alzimerahmed84.keyboard.latin.utils.getTimestampFormatter
...
class InputLogicTest {
    ...
    //  https://github.com/alzimerahmed84/TypeCraft/issues/214
    ...
    @Test fun `immediate text expansion uses full segmented thai word`() {
        ...
        latinIME.prefs().edit().apply {
            putBoolean(alzimerahmed84.keyboard.latin.utils.TextExpanderUtils.PREF_ENABLED, true)
            putBoolean(alzimerahmed84.keyboard.latin.utils.TextExpanderUtils.PREF_IMMEDIATE, true)
        }.commit()
        val shortcuts = mapOf("ภาษาไทย" to alzimerahmed84.keyboard.latin.utils.TextExpanderUtils.ShortcutEntry("expanded", ""))
        alzimerahmed84.keyboard.latin.utils.TextExpanderUtils.saveShortcuts(latinIME, shortcuts)
    ...
    @Test fun `immediate text expansion still segments thai non-shortcut`() {
        ...
        val shortcuts = mapOf("อื่น" to alzimerahmed84.keyboard.latin.utils.TextExpanderUtils.ShortcutEntry("expanded", ""))
        alzimerahmed84.keyboard.latin.utils.TextExpanderUtils.saveShortcuts(latinIME, shortcuts)
    ...
    @Test fun `failed immediate expansion commits thai segments separately`() {
        ...
        val shortcuts = mapOf("ภาษาไทยดี" to alzimerahmed84.keyboard.latin.utils.TextExpanderUtils.ShortcutEntry("expanded", ""))
        alzimerahmed84.keyboard.latin.utils.TextExpanderUtils.saveShortcuts(latinIME, shortcuts)
    ...
    @Test fun `immediate regex expansion triggers for symbol typed alone`() {
        ...
        val shortcuts = mapOf("${alzimerahmed84.keyboard.latin.utils.TextExpanderUtils.REGEX_PREFIX}∆+" to alzimerahmed84.keyboard.latin.utils.TextExpanderUtils.ShortcutEntry("Expanded Text", ""))
        alzimerahmed84.keyboard.latin.utils.TextExpanderUtils.saveShortcuts(latinIME, shortcuts)
    ...
    @Test fun `immediate regex expansion triggers for currency symbol typed alone`() {
        ...
        val shortcuts = mapOf("${alzimerahmed84.keyboard.latin.utils.TextExpanderUtils.REGEX_PREFIX}£+" to alzimerahmed84.keyboard.latin.utils.TextExpanderUtils.ShortcutEntry("Pound Sterling", ""))
        alzimerahmed84.keyboard.latin.utils.TextExpanderUtils.saveShortcuts(latinIME, shortcuts)
    ...
    @Test fun `immediate regex expansion triggers for repeated symbols`() {
        ...
        val shortcuts = mapOf("${alzimerahmed84.keyboard.latin.utils.TextExpanderUtils.REGEX_PREFIX}∆{2,}" to alzimerahmed84.keyboard.latin.utils.TextExpanderUtils.ShortcutEntry("Multiple Deltas", ""))
        alzimerahmed84.keyboard.latin.utils.TextExpanderUtils.saveShortcuts(latinIME, shortcuts)
    ...
    @Test fun `immediate regex expansion triggers for capture groups`() {
        ...
        val shortcuts = mapOf("${alzimerahmed84.keyboard.latin.utils.TextExpanderUtils.REGEX_PREFIX}(\\w+)@gm" to alzimerahmed84.keyboard.latin.utils.TextExpanderUtils.ShortcutEntry("$1@gmail.com", ""))
        alzimerahmed84.keyboard.latin.utils.TextExpanderUtils.saveShortcuts(latinIME, shortcuts)
    ...
    @Test fun immediateRegexExpansionTriggersForSymbolPrefixedRegex() {
        ...
        alzimerahmed84.keyboard.latin.utils.TextExpanderUtils.clearCache()
        val shortcuts = mapOf("${alzimerahmed84.keyboard.latin.utils.TextExpanderUtils.REGEX_PREFIX}@john" to alzimerahmed84.keyboard.latin.utils.TextExpanderUtils.ShortcutEntry("user_mention", ""))
        alzimerahmed84.keyboard.latin.utils.TextExpanderUtils.saveShortcuts(latinIME, shortcuts)
        alzimerahmed84.keyboard.latin.utils.TextExpanderUtils.clearCache()
...
//        assertEquals("example.cz", composingText) // fails, but probably would be better than above
    ...
    // https://github.com/alzimerahmed84/TypeCraft/issues/215
    // https://github.com/alzimerahmed84/TypeCraft/issues/229
    ...
    // https://github.com/alzimerahmed84/TypeCraft/issues/230
    ...
    // emoRegex update to unicode 16.0 was required, https://github.com/alzimerahmed84/TypeCraft/issues/1760
    ...
    @Test fun testTextExpanderPlaceholders() {
        ...
        // Define a shortcut
        val shortcuts = mapOf("exp" to alzimerahmed84.keyboard.latin.utils.TextExpanderUtils.ShortcutEntry("Hi %cursor1%,your order %cursor2% is ready for %cursor3%.", ""))
        alzimerahmed84.keyboard.latin.utils.TextExpanderUtils.saveShortcuts(latinIME, shortcuts)
